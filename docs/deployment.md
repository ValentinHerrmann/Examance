...
- Preview branches: **only** `preview` (Settings → Builds → "Include only certain branches"). This keeps every feature branch from consuming build minutes and gives preview a stable URL, `preview.examance.pages.dev`, already covered by `CORS_ALLOWED_ORIGIN_REGEX`.
