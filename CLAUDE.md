...
- **Delegate execution to Antigravity**: Aggressively hand-off research and implementation work to Antigravity using Flash tier Model Gemini 3.8 Flash Medium (the `antigravity-delegate` agent / `antigravity:delegate` skill (Antigravity plugin, agy/Gemini), gemini-3.8-flash-medium) as subagent and focus your own capacity to planning, delegating and orchestrating instead of editing or crawling files directly. Small, self-contained, or judgement-heavy tasks stay direct. Always verify the returned digest against the actual `git diff` before reporting success — the digest is a claim, not verification. No worktree isolation — user manages git/branching.
