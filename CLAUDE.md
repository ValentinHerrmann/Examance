...
- **Delegate execution to Antigravity**: once a plan is approved and out of plan mode, prefer delegating well-scoped, above-break-even implementation work (scaffolding, exhaustive tests, migrations, digests) to the `antigravity-delegate` agent / `antigravity:delegate` skill (Antigravity plugin, agy/Gemini) instead of editing directly. Small, self-contained, or judgement-heavy tasks stay direct. Always verify the returned digest against the actual `git diff` before reporting success — the digest is a claim, not verification. No worktree isolation — user manages git/branching. Planning itself uses Claude directly; there is no config-level mode-conditional model switch (unlike the built-in `opusplan` alias, this pairing isn't a settings.json primitive) — it's this standing instruction.
