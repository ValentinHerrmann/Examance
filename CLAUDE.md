...
- **Components**: new ones → `src/lib/components/<feature>/`. Shared primitives live in `src/lib/components/ui/` (see **Design system** below). Nine legacy components sit loose at `components/` root (`ConfirmDialog`, `DualPdfPreview`, `ExerciseEditorModal`, `GradingKeyEditor`, `LatexEditor`, `LatexViewer`, `SessionTimeoutWarning`, `StoragePolicyModal`, `ZoomableImage`) — leftovers, don't copy. Routes hold data-loading, handlers, session state; components hold markup. Wire with callback props (`onAction={handler}`, `bind:value`), **not** `createEventDispatcher` — 5 legacy roots still use it (`ConfirmDialog`, `DualPdfPreview`, `ExerciseEditorModal`, `LatexEditor`, `StoragePolicyModal`); don't follow. Prop-drilling exception: `src/lib/grading/gradingStore.ts`, leaf grading components subscribe directly (15+ interdependent fields, justified in-file).
- **Design system** (2026-08-20 responsive overhaul): **inline Tailwind utilities in markup** are the house dialect. A component's own `<style>` block is for the rare rule utilities cannot express (a shared class used by many sibling elements, a keyframe). Sibling `.css` files are legacy — convert the file's component and delete the `.css` when you touch it; never add a new one.
  - **Tokens** live in one `@theme` block in `src/app.css`: surfaces `surface-base|raised|sunken|inset`, `line`/`line-strong`, `accent`/`accent-strong`/`accent-hover`, text `content`/`muted`/`subtle`, content widths `max-w-form|narrow|wide|full`, and a z-index ladder as CSS vars (`--z-dropdown` 1000 / `--z-modal` 2000 / `--z-toast` 3000). Use the token utility (`bg-surface-raised`), not the raw hex or the underlying Tailwind shade. Tailwind v4 emits a theme variable only if it finds it spelled out in source — a token only ever referenced as a runtime-built `var(--x-${n})` string must live in an `@theme static` block (see the grade colours).
  - **Primitives** in `src/lib/components/ui/`: `Modal` (every dialog goes through it — it owns backdrop, z-index, Escape, focus trap, scroll lock, and the phone full-sheet shape), `Button`, `Card`, `Field`/`TextInput`/`Select` (+ `controlClass`), `PageShell` (the one page root), `PageHeader`, `TableScroller` (any wide table). Import from `$lib/components/ui`.
...
- **Delegate execution to Antigravity**: Aggressively hand-off research and implementation work to Antigravity using Flash tier Model Gemini 3.8 Flash Medium (the `antigravity-delegate` agent / `antigravity:delegate` skill (Antigravity plugin, agy/Gemini), gemini-3.8-flash-medium) as subagent and focus your own capacity to planning, delegating and orchestrating instead of editing or crawling files directly. Small, self-contained, or judgement-heavy tasks stay direct. Make sure to give the antigravity subagent all relevant context and precise instructions to avoid unneccessary mistakes. Always verify the returned digest against the actual `git diff` before reporting success — the digest is a claim, not verification. No worktree isolation — user manages git/branching.
